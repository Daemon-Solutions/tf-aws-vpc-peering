...
resource "aws_vpc_peering_connection" "peering" {
  ...
  peer_vpc_id     = "${data.terraform_remote_state.destination_remote_state.vpc_id}"
...
resource "aws_route" "peering_routes" {
  ...
  destination_cidr_block    = "${data.terraform_remote_state.destination_remote_state.vpc_cidr}"
